...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Diagnose a World Cup 2026 group under the baseline, hybrid, "
            "and calibrated match-model recipe."
        )
    )
    ...
    parser.add_argument(
        ...
        help="Training frame used to fit the optional Elo benchmark blend.",
    )
    parser.add_argument(
        "--elo-blend-alpha",
        type=float,
        default=1.0,
        help="Weight on baseline Dixon-Coles probabilities.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Adds scale * squad strength to team Elo ratings during group diagnosis.",
    )
    parser.add_argument(
        "--secondary-squad-elo-scale",
        type=float,
        default=0.0,
        help="Adds scale * secondary squad strength to team Elo ratings during group diagnosis.",
    )
    ...
    parser.add_argument(
        "--output",
        default="reports/group_model_diagnosis.json",
    )
    return parser
...
def main() -> None:
    ...
    team_strength_ratings = _load_team_strength_ratings(
        Path(args.squad_strength_input) if args.squad_strength_input else None,
        model=model,
...
def _load_team_strength_ratings(
    ...
    *,
    ...
    ratings_frame["team"] = ratings_frame["team"].map(lambda team: model_team_lookup.get(team, team))
    return {
        str(row.team): float(getattr(row, rating_column))
